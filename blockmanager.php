...
// Handle blocking/unblocking actions
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && isset($_POST["email"])) {
    $action = $_POST["action"];
    $email = $_POST["email"];

    // Perform the blocking or unblocking action in the database
    if ($action == "block") {
        $updateSql = "UPDATE managers SET blocked = 1 WHERE email = '$email'";
    } elseif ($action == "unblock") {
        $updateSql = "UPDATE managers SET blocked = 0 WHERE email = '$email'";
    }
